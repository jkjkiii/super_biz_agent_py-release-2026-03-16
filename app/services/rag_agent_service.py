...
class RagAgentService:
    ...
    async def query(self, question: str, session_id: str, user_id: Optional[str] = None) -> str:
        try:
            ...
            logger.info(f"[会话 {session_id}] 用户标识: {actual_user_id}")
    ...
    async def query_stream(
        ...
        try:
            ...
            if route_result.intent == Intent.TROUBLESHOOT:
                ...
                async for chunk in route_handlers.handle_troubleshoot_stream(question, actual_user_id):
                    # 转发所有事件类型（intent/tool_call/content），前端才能实时显示状态
            ...
            # ====== 闲聊（流式） ======
            if route_result.intent == Intent.CHAT and route_result.confidence > 0.6:
                logger.info(f"[会话 {session_id}] 🟢 硬路由 → 闲聊（流式）")
                async for chunk in route_handlers.handle_chat_stream(question, actual_user_id):
                    # 转发所有事件类型（intent/content），前端才能实时显示状态
                    if chunk.get("type") == "content":
                        full_answer += chunk.get("data", "")
                    yield chunk
                if actual_user_id and full_answer:
            ...
            # ====== 知识查询（流式） ======
            if route_result.intent == Intent.KNOWLEDGE and route_result.confidence > 0.7:
                logger.info(f"[会话 {session_id}] 🟢 硬路由 → 知识查询（流式）")
                async for chunk in route_handlers.handle_knowledge_stream(question, actual_user_id):
                    # 转发所有事件类型（intent/content），前端才能实时显示状态
                    if chunk.get("type") == "content":
                        full_answer += chunk.get("data", "")
                    yield chunk
            ...
            if route_result.intent == Intent.CONTROL and route_result.confidence > 0.7:
                ...
                async for chunk in route_handlers.handle_control_stream(question, actual_user_id):
                    # 转发所有事件类型（intent/content），前端才能实时显示状态
                    if chunk.get("type") == "content":
                        full_answer += chunk.get("data", "")
                    yield chunk
